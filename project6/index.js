...
app.get('/jokes/:id', (req, res) => {
  ...
  const specificJok = jokes.filter((item) => item.id === jokId);
  res.send(specificJok)
})

app.get('/filter', (req, res) => {
  const jokType = req.query.type;
  console.log(jokType);
  const result = jokes.filter((item) => jokType === item.jokeType);
  res.send(result);
});
...
app.put('/jokes/:id', (req, res) => {
  ...
  const currentJoke = jokes.find((item) => id === item.id);
  jokes[currentJoke] = data;
  res.send(jokes[currentJoke])
})
app.delete('/jokes/:id', (req, res) => {
  const jokId = parseInt(req.params.id);
  const searchIndex = jokes.findIndex((joke) => joke.id === jokId);
  if (searchIndex > -1) {
